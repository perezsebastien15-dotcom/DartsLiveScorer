...
public class ModifyPlayerActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        modifier.setOnClickListener(v -> modifierJoueur(playerId, nom.getText().toString().trim()));
    }

    private void modifierJoueur(long id, String nomSaisi) {
        if (nomSaisi.isEmpty()) {
        ...
        executor.execute(() -> {
            try {
                ...
                DartScorerDatabase.getDatabase(getApplicationContext())
                        .dartScorerDao().updateJoueur(joueur);
